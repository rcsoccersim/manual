...
How to connect to an online coach.By default, port 6002 is used Online_coach. You need to pay attention to the following points when connecting: 
...
* **(3)**  type Online_coach needs to send initialization information to the server, including its own team name, that is, to identify the server, otherwise the server will not be processed, and also specify the protocol version used.
