...
.. figure:: ./images/view-example.*
  ...
  Only objects within ``view_angle``/2, and those within ``visible_distance``
  of the viewing agent can be seen. ``unum_far_length``, ``unum_too_far_length``,
  ``team_far_length``, and ``team_too_far_length`` affect the amount of precision
  with which a players’ identity is given. Taken from [19].
